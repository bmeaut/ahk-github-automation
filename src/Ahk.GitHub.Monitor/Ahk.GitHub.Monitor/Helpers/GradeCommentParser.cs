...
{
    private static readonly Regex CommandRegex = new(@"^/ahk ok($|(\s.*))",
        RegexOptions.IgnoreCase | RegexOptions.Singleline | RegexOptions.Compiled);

    private static readonly Regex GradesRegex = new(@"[0-9]+([,\.][0-9]{1,3})?",
        RegexOptions.IgnoreCase | RegexOptions.Singleline | RegexOptions.Compiled);
    ...
    {
        MatchCollection gradesMatch = GradesRegex.Matches(value);
