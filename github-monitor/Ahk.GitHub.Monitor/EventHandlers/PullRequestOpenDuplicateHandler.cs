    ...
    : RepositoryEventBase<PullRequestEventPayload>(gitHubClientFactory, cache, serviceProvider)
{
    ...
    protected override async Task<EventHandlerResult> executeCore(PullRequestEventPayload webhookPayload)
    {
        ...
        {
            IReadOnlyList<PullRequest> repositoryPrs = await this.getAllRepoPullRequests(webhookPayload);
    ...
    private Task<IReadOnlyList<PullRequest>> getAllRepoPullRequests(PullRequestEventPayload webhookPayload)
        =>
            this.GitHubClient.PullRequest.GetAllForRepository(webhookPayload.Repository.Id,
        ...
        PullRequestEventPayload webhookPayload, IReadOnlyCollection<PullRequest> repositoryPrs)
    {
        ...
        if (openPrs.Count > 1)
        {
            ...
            foreach (PullRequest openPullRequest in openPrs)
            {
                await this.GitHubClient.Issue.Comment.Create(webhookPayload.Repository.Id, openPullRequest.Number,
                    warningText);
            }
        ...
        PullRequestEventPayload webhookPayload, IReadOnlyCollection<PullRequest> repositoryPrs)
    {
        ...
        if (closedPrs.Count != 0)
        {
            ...
            foreach (PullRequest otherClosedPr in closedPrs)
            {
                if (await this.isPrClosedByNotStudent(webhookPayload, otherClosedPr))
                {
            ...
            if (prsClosedByNotStudent.Count != 0)
            {
                ...
                await this.GitHubClient.Issue.Comment.Create(webhookPayload.Repository.Id, webhookPayload.Number,
                    warningText);
    ...
    {
        IReadOnlyList<IssueEvent> issueEvents =
            await this.GitHubClient.Issue.Events.GetAllForIssue(webhookPayload.Repository.Id, pr.Number);
